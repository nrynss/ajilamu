...
func Export(video, audio, output string) error {
	if video == "" || audio == "" {
	...
	if err := requireVideoStream(video); err != nil {
		return err
	}
	format, err := filmAudioFormat(video, audio)
	if err != nil {
		return err
	}
	...
	duration, err := probeFormatSeconds(video)
	if err != nil {
	...
	return renderExport(video, audio, output, format, duration)
}

func filmAudioFormat(video, audio string) (media.Format, error) {
	if format, err := media.AudioFormat(video); err == nil {
		return format, nil
	}
	format, err := media.AudioFormat(audio)
	if err != nil {
...
func requireVideoStream(path string) error {
	out, err := probeTool("-v", "error", "-select_streams", "v:0",
...
func probeFormatSeconds(path string) (float64, error) {
	raw, err := probeTool("-v", "error", "-show_entries", "format=duration",
...
func probeTool(args ...string) (string, error) {
...
func renderExport(video, audio, output string, format media.Format, duration float64) error {
